...
var volumeOn = true;  // Toujours activé
var nbLettres = 6;    // Nombre de lettres par défaut à 6
...
function nouvelEssai() {
    ...
    if (nbEssais <= NB_ESSAIS_MAX) {
        ...
        sonPerdu.onended = function() {
            if (confirm("Perdu ! Voulez-vous réessayer ?")) {
                nouvellePartie();
            } else {
                nbEssais = 0;
            }
...
sonMotTrouve.onended = function(event) {
    if (confirm("Bien joué !\nUne nouvelle partie ? Va le voir !")) nouvellePartie()
}
